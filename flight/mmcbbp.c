...
#define	INIT_PORT()	DDRB = B00101001	/* Initialize MMC control port (CS/CLK/DI:output, DO:input) */
#define DLY_US(n)	delayMicroseconds(n)		/* Delay n microseconds */
#define	FORWARD(d)	;	/* Data in-time processing function (depends on the project) */

// for Sparkfun shield: CS = D8, SCLK = D13, DI = D11, DO = D12
#define	CS_H()		PORTB |= B00000001	/* Set MMC CS "high" */
#define CS_L()		PORTB &= B11111110	/* Set MMC CS "low" */
#define CK_H()		PORTB |= B00100000	/* Set MMC SCLK "high" */
#define	CK_L()		PORTB &= B11011111	/* Set MMC SCLK "low" */
#define DI_H()		PORTB |= B00001000	/* Set MMC DI "high" */
#define DI_L()		PORTB &= B11110111	/* Set MMC DI "low" */
#define DO		((PINB>>4) & 0x01) 	/* Test MMC DO (high:true, low:false) */
